...
async (conn, mek, c, { from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    if (!isOwner) return reply("❌ You are not the owner!");
...
async (conn, mek, c, { from, isOwner, reply, args }) => {
    if (!isOwner) return reply("❌ You are not the owner!");
...
chris({
    ...
    category: "owner",
    filename: __filename
}, async (conn, mek, c, { quoted, isGroup, sender, participants, reply }) => {
    try {
