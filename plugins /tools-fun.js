...
async (conn, mek, c, { from, isGroup, text, reply }) => {
    try {
        ...
        const mentionedUser = c.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
        if (!mentionedUser) {
...
chris({
  ...
  category: "fun",
  filename: __filename
}, async (conn, c, store, { args, reply, senderNumber }) => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      await conn.sendMessage(c.from, { text: message }, { quoted: c });
...
chris({
  ...
  try {
    ...
    await conn.sendMessage(m.from, { text: message }, { quoted: c });
  } catch (error) {
