...
chris({
  on: "body"
}, async (conn, c, { isGroup }) => {
  try {
    if (config.MENTION_REPLY !== 'true' || !isGroup) return;
    if (!c.mentionedJid || m.mentionedJid.length === 0) return;
    ...
    if (m.mentionedJid.includes(botNumber)) {
      ...
      await conn.sendMessage(m.chat, {
        ...
        contextInfo: {
          ...
          isForwarded: true,
          externalAdReply: {
            title: config.BOT_NAME || "CHRISM-XD",
            body: config.DESCRIPTION || "POWERED BY CHRIS XD",
            mediaType: 1,
            renderLargerThumbnail: true,
            thumbnail: thumbnailBuffer,
            mediaUrl: "https://files.catbox.moe/medum3", // Static image URL
            sourceUrl: "https://wa.me/2348069675806",
