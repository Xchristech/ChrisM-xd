...
chris({
    ...
    filename: __filename
}, async (conn, mek, c, { from, reply, q }) => {
    try {
        if (!q && !c.quoted) return reply("❓ What song or URL do you want to download? You can also reply to a message with a URL.");
        
        let input = q || (c.quoted && c.quoted.text);
