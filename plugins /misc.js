...
async (conn, mek, c, { from, reply, q, text, isCreator, fromMe }) => {
...
async (conn, mek, c, { from, reply }) => {
    try {
        const quotedMessage = c.msg.contextInfo.quotedMessage; // Get quoted message
        ...
        if (!c.quoted) return reply("Please reply to a ViewOnce message.");
        if (m.quoted.mtype === "viewOnceMessage") {
            if (m.quoted.message.imageMessage) {
                let cap = c.quoted.message.imageMessage.caption;
                let anu = await conn.downloadAndSaveMediaMessage(m.quoted.message.imageMessage);
                return conn.sendMessage(from, { image: { url: anu }, caption: cap }, { quoted: mek });
            }
            else if (m.quoted.message.videoMessage) {
                let cap = c.quoted.message.videoMessage.caption;
