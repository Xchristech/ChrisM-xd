...
// Fixed & Created By Chris x bethel 
chris({
  pattern: "hidetag",
  alias: ["tag", "h"],  
  react: "🔊",
...
async (conn, mek, c, {
  ...
  try {
    ...
    if (c.quoted) {
      const type = c.quoted.mtype || '';
      
      // If it's a text message (extendedTextMessage)
      if (type === 'extendedTextMessage') {
        return await conn.sendMessage(from, {
          text: c.quoted.text || 'No message content found.',
          ...mentionAll
        }, { quoted: mek });
      }
      ...
      if (['imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage', 'documentMessage'].includes(type)) {
        try {
          const buffer = await c.quoted.download?.();
          ...
          switch (type) {
            ...
            case "documentMessage":
              content = {
                document: buffer,
                mimetype: c.quoted.message?.documentMessage?.mimetype || "application/octet-stream",
                fileName: c.quoted.message?.documentMessage?.fileName || "file",
                caption: c.quoted.text || "",
                ...mentionAll
              };
              break;
          }
